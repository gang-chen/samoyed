...
#ifndef SMYD_SOURCE_IMAGE_HPP
#define SMYD_SOURCE_IMAGE_HPP
...
/**
 * A source image represents the source in a source file.  It regulates all the
 * accesses to the stored data.
 *
 * A source image is multithreading safe.  A read-write mutex is used to protect
 * it.
 *
 * A source image is observable.  An observer can be added by registering a
 * callback to a source image.  The callback will be called after every change
 * on the source image occurs.
 */
