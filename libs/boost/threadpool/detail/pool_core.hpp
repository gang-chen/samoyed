...
// TODO is only called once
    ...
    */  
    ...
    /*! Returns the highest priority of the pending tasks.
    * Only applicable to prio_scheduler.
    */
    unsigned int highest_pending_priority() const volatile
    {
      locking_ptr<const pool_type, recursive_mutex> lockedThis(*this, m_monitor);
      return lockedThis->m_scheduler.top().priority();
    }
